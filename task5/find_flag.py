...
# The check offsets from the disassembly (which input position each check validates)
check_offsets = [
...
# Apply XOR with stride 17 to decrypt expected values
pos = 0
for i in range(27):
    if pos < len(code):
        code[pos] ^= xor_key[i]
...
# Build the flag by mapping each decrypted char to its input position
